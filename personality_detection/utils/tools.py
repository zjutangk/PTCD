...
def evaluate(Y_test, scores, threshold=0.5,num_label=4):
    return metric_new(Y_test, 1 * (scores > threshold), scores,num_label=num_label)


def metric(y_true, y_pred, scores):
    print("eval_num", y_true.size)
    print("y_true,y_pred", y_true[:5], y_pred[:5])
    f1_C = f1_score(y_true, y_pred, average="macro")
    num_same_elements = np.sum(y_true == y_pred)
    total = y_pred.size
    acc = num_same_elements / total

    # exact_acc
    correct_predictions = np.all(y_true == y_pred, axis=1)
    exact_accuracy = np.mean(correct_predictions)
    return acc, exact_accuracy, f1_C


def metric_new(y_true, y_pred, scores,num_label=4):
    print("eval_num", y_true.size)
    print("y_true,y_pred", y_true[:5], y_pred[:5])
    ...
    dimension_metrics = []
    for dim in range(num_label):
        y_true_dim = y_true[:, dim]
        y_pred_dim = y_pred[:, dim]

        dim_acc = np.mean(y_true_dim == y_pred_dim)
        if dim == 0:
            print("y_true_dim,y_pred_dim", y_true_dim[:5], y_pred_dim[:5])
        dim_f1 = f1_score(y_true_dim, y_pred_dim, average="macro")

        dimension_metrics.append((dim_acc, dim_f1))
    ...
    return {
        ...
        "macro_f1": f1_C,
        "dimension_metrics": {
            f"dim_{i}": {"accuracy": metrics[0], "f1_score": metrics[1]}
            for i, metrics in enumerate(dimension_metrics)
        },
    }
